...
const metrics = [
  { label: 'CAPTCHA Required', value: '312', delta: '+18 from yesterday', action: 'View all', accent: 'amber', icon: ShieldCheck },
  { label: 'Failed Tracking', value: '487', delta: '+25 from yesterday', action: 'View all', accent: 'red', icon: Link2Off },
  { label: 'Manual ETA Missing', value: '256', delta: '+12 from yesterday', action: 'View all', accent: 'violet', icon: CalendarClock },
  { label: 'Near Arrival (72h)', value: '184', delta: '+7 from yesterday', action: 'View all', accent: 'cyan', icon: Ship },
] as const;

const rows = [
  { id: 'TCLU 8567421', tag: 'CAPTCHA', company: 'Evergreen Marine', route: 'Shanghai, CN -> Los Angeles, US', carrier: 'EVERGREEN', voyage: 'AE123W', eta: 'May 28, 2025', age: '(2 days ago)', status: 'CAPTCHA detected', detail: 'Interaction required', action: 'Capture', accent: 'amber' },
  { id: 'MSCU 7891234', tag: 'FAILED TRACKING', company: 'Walmart Global', route: 'Singapore, SG -> New York, US', carrier: 'MSC', voyage: 'SWX12R', eta: 'May 26, 2025', age: '(4 days ago)', status: 'Extraction failed', detail: 'Structure changed', action: 'Retry', accent: 'red' },
  { id: 'CMAU 1234567', tag: 'MANUAL ETA', company: 'Target Corporation', route: 'Ningbo, CN -> Savannah, US', carrier: 'CMA CGM', voyage: 'AA47W', eta: 'No ETA available', age: 'MISSING', status: 'Manual ETA missing', detail: 'Provide estimated arrival', action: 'Enter Manual ETA', accent: 'violet' },
  { id: 'HLCU 4567890', tag: 'NEAR ARRIVAL', company: 'Home Depot', route: 'Busan, KR -> Long Beach, US', carrier: 'Hapag-Lloyd', voyage: 'TN25W', eta: 'May 31, 2025', age: '(1 day)', status: 'Arriving in 36 hours', detail: 'Monitor for updates', action: 'Track Now', accent: 'cyan' },
  { id: 'ZCSU 9876543', tag: 'FAILED TRACKING', company: 'Costco Wholesale', route: 'Yantian, CN -> Seattle, US', carrier: 'ZIM', voyage: 'ZIM7W', eta: 'May 25, 2025', age: '(5 days ago)', status: 'Extraction failed', detail: 'Timeout waiting for data', action: 'Retry', accent: 'red' },
  { id: 'OOLU 7654321', tag: 'CAPTCHA', company: "Lowe's Companies", route: 'Kaohsiung, TW -> Oakland, US', carrier: 'OOCL', voyage: 'TPA3W', eta: 'May 27, 2025', age: '(3 days ago)', status: 'CAPTCHA detected', detail: 'Interaction required', action: 'Capture', accent: 'amber' },
] as const;

const accentClasses = {
  amber: { border: 'border-l-amber-400', text: 'text-amber-300', bg: 'bg-amber-400/15', chip: 'bg-amber-500/20 text-amber-200', button: 'bg-blue-600 hover:bg-blue-500' },
  red: { border: 'border-l-red-400', text: 'text-red-300', bg: 'bg-red-400/15', chip: 'bg-red-500/20 text-red-200', button: 'bg-blue-600 hover:bg-blue-500' },
  violet: { border: 'border-l-violet-400', text: 'text-violet-300', bg: 'bg-violet-400/15', chip: 'bg-violet-500/20 text-violet-200', button: 'bg-violet-600 hover:bg-violet-500' },
  cyan: { border: 'border-l-cyan-400', text: 'text-cyan-300', bg: 'bg-cyan-400/15', chip: 'bg-cyan-500/20 text-cyan-200', button: 'bg-blue-600 hover:bg-blue-500' },
} as const;
    ...
    <div className="space-y-4 text-slate-100">
      <section className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex items-center gap-4">
          <div className="grid h-14 w-14 place-items-center rounded-lg border border-blue-400/40 bg-blue-500/15 text-blue-200 shadow-lg shadow-blue-950/40">
      ...
      <section className="queue-panel overflow-hidden">
          ...
          <table className="w-full min-w-[1120px] text-left">
            ...
            <tbody className="divide-y divide-slate-700/60">
              {rows.map((row) => {
                ...
                return (
                  <tr key={row.id} className={`border-l-4 ${accent.border} bg-slate-900/30 transition hover:bg-slate-800/50`}>
        ...
        <div className="flex flex-col gap-3 border-t border-slate-700/60 px-5 py-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-2">
            ...
            {[1, 2, 3, 4, 5].map((pageNumber) => <button key={pageNumber} className={`pagination-button ${pageNumber === 1 ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}>{pageNumber}</button>)}
